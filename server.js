...
cron.schedule('* * * * *', async () => {
    ...
    try {
        const activeTasks = await Task.find({ isCompleted: false, dueDate: { $gt: now } }).populate('user', 'pushSubscription email');
    ...
    } catch (error) { console.error('❌ Lỗi nghiêm trọng trong cron job:', error); }
...
async function sendNotificationForTask(task, type, reminderKey = '') {
    ...
    let bodyText = '';
    let notifiedKey = ''; // Key để cập nhật vào Map 'notified'
    ...
    if (type === 'upcoming') {
        ...
        notifiedKey = `upcoming_${reminderKey}`; // Chính xác: upcoming_5m, upcoming_1h,...
    } else { // type === 'due'
        bodyText = `Đã đến hạn hoàn thành nhiệm vụ "${task.title}"!`;
        notifiedKey = 'due'; // Chính xác: due
    }
    ...
    try {
        ...
        // SỬA LỖI TẠI ĐÂY: Sử dụng notifiedKey đã xác định ở trên
